...
class Settings(BaseSettings):
    ...
    # Что делать с чатом, которого нет в базе:
    #   auto — доверять базе, но только если она реально загружена (дефолт)
    #   open — всегда считать первым контактом и отвечать
    #   safe — всегда считать старым и молчать
    # Смысл auto: если baseline из amoCRM загрузился, «нет в базе» честно
    # означает «новый клиент» и боту можно отвечать. А если импорт упал или
    # база пустая — «нет в базе» не значит ничего, и мы молчим, вместо того
    # чтобы поздороваться со всей клиентской базой разом.
    first_contact_policy: str = "auto"
    # Сколько «старых» чатов должно быть известно, чтобы поверить базе.
    # У клиента их около 2800; полсотни — заведомо не случайность.
    baseline_min_chats: int = 50
