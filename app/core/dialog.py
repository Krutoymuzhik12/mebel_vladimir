...
"""
    ...
    async def handle(
        ...
        # Маркер, которого код не знает, вырезается из текста и молча пропадает.
        # Ровно так [[СОТРУДНИК]] делал вид, что работает: бот обещал клиенту
        # передать вопрос, а не передавал никуда. Пусть расхождение промпта с
        # кодом хотя бы видно в логе.
        unhandled = [*markers.unknown, *(["СОТРУДНИК"] if markers.staff else [])]
        if unhandled:
            logger.warning(
                "chat=%s модель поставила маркер, который код не обрабатывает: %s "
                "— проверьте промпт на Poe",
                chat_id,
                ", ".join(unhandled),
            )
